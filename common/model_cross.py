...
def plot_hot(data, filename):
    ...
    if not 'temp' in filename:
        ...
        ax.set_yticklabels(np.arange(0, data.shape[0]))
    ...
    # 顯示繪製的圖表
    plt.savefig('vis_correlation/' + filename)
...
class Attention(nn.Module):
    ...
    def forward(self, x, vis=False):
        ...
        if record == 'spacial':
            attn2 = attn.cpu().numpy().copy()
            attn2 = attn2[..., :17, :17]
            attn_avg = np.mean(attn2, 0)
            attn_avg = np.mean(attn_avg, 0)
            
            if attn.shape[2] > 17:
                np.savez("vis_correlation/data_17+n_spa_corre.npz", data = attn_avg)
                hot_filename = "heatmap_17+n_spa_corre.png"
            else:
                np.savez("vis_correlation/data_17+0_spa_corre.npz", data = attn_avg)
                hot_filename = "heatmap_17+0_spa_corre.png"

            min_a, max_a = np.min(attn_avg), np.max(attn_avg)
            attn_norm = (attn_avg - min_a) / (max_a-min_a)

            plot_hot(attn_norm, hot_filename)

        elif record == 'temporal':
            attn2 = attn.cpu().numpy().copy()
            if attn2.shape[0] % 17 != 0:
                select = [i for i in range(attn2.shape[0]) if i % 22 not in [17, 18, 19, 20, 21]]
                attn2 = attn2[select]
            # else:
            #     select = [i for i in range(attn2.shape[0]) if i % 17 == 16]
            #     attn2 = attn2[select]
                
            attn_avg = np.mean(attn2, 0)
            attn_avg = np.mean(attn_avg, 0)

            if attn.shape[0] % 17 == 0:
                np.savez("vis_correlation/data_17+0_temp_corre.npz", data = attn_avg)
                hot_filename = "heatmap_17+0_temp_corre.png"
            else:
                np.savez("vis_correlation/data_17+n_temp_corre.npz", data = attn_avg)
                hot_filename = "heatmap_17+n_temp_corre.png"

            min_a, max_a = np.min(attn_avg), np.max(attn_avg)
            attn_norm = (attn_avg - min_a) / (max_a-min_a) # compress between 0 and 1
            plot_hot(attn_norm, hot_filename)
...
class  MixSTE2(nn.Module):
    ...
    def ST_foward(self, x, is_record):
        ...
        for i in range(1, self.block_depth):
            ...
            global record
            if i == self.block_depth-1 and is_record:
                record = 'spacial'
